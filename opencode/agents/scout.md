...
2. The user's local docs directory (`~/Docs/<name>/` per their convention). Pick the repo that matches the topic and grep/read it. The orchestrator may name a specific repo — if so, go straight to it.
