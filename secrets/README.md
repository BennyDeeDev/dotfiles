...
- **Host decrypt identity**: split across two layers when Home Manager is used:
  ...
  - Home level (HM hosts only): standalone age key at `~/.config/sops/age/keys.txt`, auto-generated by the HM sops module on first HM activation if missing (`sops.age.generateKey = true` in `profiles/home/common/workstation/sops.nix`). Used by HM `sops-nix.service` to decrypt home-level secrets (rclone, etc.).
...
- `AGE-PLUGIN-YUBIKEY-1...` line — the identity (pointer to slot, NOT the private key). Goes into `profiles/home/common/workstation/sops.nix`'s `identity.txt`.
...
**First host only (fleet bootstrap)**: sops-nix's `generate-age-key` activation script is gated behind declared secrets. With zero secrets declared, it never fires. So the very first host needs manual keygen to break the chicken-and-egg — this happens exactly once across the lifetime of the fleet:
...
In `hosts/desktop/configuration.nix`, override the shared `sops.age.keyFile` from `modules/nixos/sops.nix`:
