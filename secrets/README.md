...
- **Host decrypt identity**: split across two layers when Home Manager is used:
  ...
  - Home level (HM hosts only): standalone age key at `~/.config/sops/age/keys.txt`, auto-generated by the HM sops module on first HM activation if missing (`sops.age.generateKey = true` in `modules/secrets.nix`). Used by HM `sops-nix.service` to decrypt home-level secrets (rclone, etc.).
...
- `AGE-PLUGIN-YUBIKEY-1...` line — the identity (pointer to slot, NOT the private key). Goes into `modules/secrets.nix`'s `identity.txt`.
...
In `hosts/desktop/configuration.nix`, override the shared `sops.age.keyFile` from `modules/secrets.nix`:
